...
class DataHandler:
    """
    Versión 10.14: Fix Crítico de Bindings en UPDATE + Persistencia de Imágenes.
    - Corrige el error de "Incorrect number of bindings" al enviar actualizaciones.
    - Mantiene prefijos dinámicos (registro/seguimientoN).
    - Mantiene mapeo PascalCase para visualización de 'Asesora' en UI.
    """
    ...
    def enqueue_client_data(self, action_type, data):
        ...
        try:
            ...
            if action_type == "ADD":
                ...
                # El query tiene: len(params) + 2 placeholders finales
                params_for_query = params + [ref, ref]
                cursor.execute(f"UPDATE prospectos SET {', '.join(sql_parts)} WHERE id_unico = ? OR nombre = ?", params_for_query)
    ...
    def process_queue_step(self):
        ...
        if success:
            ...
            if q_type != "DELETE":
                ...
                # Si no hay id_unico, usamos el nombre del payload original
                ref_id = q_phone_id
                nombre_ref = data.get('nombre_original') or data.get('Nombre')
                cursor.execute("UPDATE prospectos SET imagenes_url = ?, validacion = 'OK' WHERE id_unico = ? OR nombre = ?", (f_url, ref_id, nombre_ref))
